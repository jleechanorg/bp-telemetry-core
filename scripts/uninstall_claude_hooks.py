...
"""
Uninstall script for Claude Code telemetry capture (Redis-based hooks).

Removes hooks from ~/.claude/hooks/telemetry/ and updates settings.json.
"""
...
def uninstall_hooks(force: bool = False) -> bool:
    """
    Uninstall hooks from ~/.claude/hooks/telemetry/ directory.
    ...
    try:
        ...
        print(f"🗑️  Removing hooks from {hooks_dir}")
        ...
        # Remove parent directory files
        hook_base = hooks_dir.parent / "hook_base.py"
        if hook_base.exists():
            hook_base.unlink()
            print(f"   ✅ Removed hook_base.py")
            removed_count += 1
        ...
        shared_dir = hooks_dir.parent / "shared"
        if shared_dir.exists():
            shutil.rmtree(shared_dir)
            print(f"   ✅ Removed shared/ (modules)")
            removed_count += 1
        ...
        if removed_count == 0:
            print("   ℹ️  No hook files found to remove")
...
def update_settings_json(backup: bool = True) -> bool:
    ...
    try:
        ...
        if removed_count == 0:
            print("   ℹ️  No hooks found in settings.json")
        else:
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description='Uninstall Claude Code telemetry capture (Redis-based hooks)'
    )
    parser.add_argument(
        '--dry-run',
        action='store_true',
    ...
    print("=" * 60)
    print("Blueplane Telemetry - Claude Code Uninstall (Redis hooks)")
    print("=" * 60)
    ...
    if args.dry_run:
        print("Would remove:")
        print("  - Hooks from ~/.claude/hooks/telemetry/")
        print("  - Hook entries from ~/.claude/settings.json")
        print("\nRun without --dry-run to proceed")
        return 0

    # Remove hooks
    print("🗑️  Uninstalling hooks...")
    ...
    print("\n💡 Redis-based hooks have been removed.")
    print("   To install HTTP-based hooks:")
    print("     python scripts/install_claude_hooks_http.py")
